...
async function refreshList(){
  try{
    ...
    // render each dir as collapsible
    Object.keys(grouped).sort().forEach(dir=>{
      const dirRow = document.createElement('div');
      dirRow.className = 'dirSection';
      const header = document.createElement('div');
      header.className = 'dirHeader';
      header.style.cursor = 'pointer';
      const displayName = dir === 'root' ? '(root)' : dir;
      header.textContent = displayName + ' (' + grouped[dir].length + ')';
      const toggle = document.createElement('button');
      toggle.textContent = '展开/折叠';
      toggle.style.marginLeft='8px';
      header.appendChild(toggle);
      dirRow.appendChild(header);
      ...
      grouped[dir].forEach(item=>{
        ...
        name.textContent = item.filename + ' (' + (Math.round((item.size||0)/1024)) + 'KB)';
        name.style.marginRight='8px';
        row.appendChild(name);
        // download/show link using /api/d/<dir>/<filename>
        const origin = window.location.origin;
        const safeDir = encodeURIComponent(dir);
        const safeName = encodeURIComponent(item.filename);
        const link = document.createElement('a');
        link.href = origin + '/api/d/' + safeDir + '/' + safeName;
        link.target = '_blank';
        link.textContent = '打开/下载';
        row.appendChild(link);
        // delete button (existing delete endpoint expects id)
        ...
        del.addEventListener('click', async ()=>{
          ...
          try{
            const r = await fetch('/api/delete', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ id: item.id }) });
      ...
      dirRow.appendChild(fileList);
      listBox.appendChild(dirRow);
    });
